...
class MCTS_RAVEPlayer(Player):
    ...
    def __init__(self, iterations = 1000, timeLimit = 10, isTimeLimited = False, c_param = 4, logs=False):
        super().__init__()
    ...
    def MCTS_Search(self, root_state, iterations, timeLimit, isTimeLimited):
        ...
        #for i in sorted(root_node.child, key = lambda c: c.Weight): print(i)
        bestMove = root_node.RAVESearch()
    ...
    def MCTS_IterationLimit(self, root_node, root_state):
        ...
        endTime = time.time()
        print(f'(MCTS_RAVE) TimeTaken: {round(endTime - startTime,3)} secs, Time:{time.strftime("%H:%M:%S", time.localtime())}')
    ...
    def MCTS_TimeLimit(self, root_node, root_state):
        ...
        while ((endTime - startTime) < self.timeLimit):
            ...
            if node.untried_moves != [] and (not state.isGameOver):  # if we can expand, i.e. state/node is non-terminal
                ...
                actions.append((state.playerSymbol, move_random.move))
                state.move(move_random.move)
                #print(f'(MCTS) Expanspion, Move: {move_random.move}')
                #print(f'(MCTS) Expanspion, Game Over: {state.isGameOver}')
                node = node.AddChild(move = move_random, state = state, isGameOver = state.isGameOver)
...
class Node:
    ...
    def RAVESearch(self, c_param=0, verbose=False):
        """
        ...
        for c in self.child:
            ...
            weight = (1-beta)*c.Q + beta*Q_AMAF + (factor * np.sqrt(c_param * np.log(self.visits) / N))
