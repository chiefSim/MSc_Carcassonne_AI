...
class Star2_5(Player):
    ...
    def __init__(self, currentPlayer = 0, Depth = 0, MaxDepth = 1, Alpha = -float('inf'), Beta = float('inf'), ProbingFactor = 3, Chance = None, logs = False):
        super().__init__()
    ...
    def ClonePlayer(self):
        Clone = Star2_5(self.currentPlayer, self.Depth, self.MaxDepth, self.Alpha, self.Beta, self.ProbingFactor, self.Chance, self.logs)
        return Clone
    ...
    def chooseAction(self, state):
        ...
        print(f'(Star2.5) TimeTaken: {round(endTime - startTime,3)} secs, Time:{time.strftime("%H:%M:%S", time.localtime())}')
        ...
        if state.isGameOver or self.Depth == self.MaxDepth:
            ...
            for i in range(len(Chances)):
                ...
                for j in range(Its):               
                    ...
                    _, TempValue, VN = Star2_5(self.currentPlayer, self.Depth +1, self.MaxDepth, AX, BX).bestMove(NewState, VN)                
                    if TempValue >= BX:
                        MoveValue = BX
                    elif TempValue > AX:
                        MoveValue = TempValue
            ...
            for i in range(len(Chances)):
                ...
                if state.playerSymbol == self.currentPlayer: 
                    ...
                    for Move in Moves: 
                        ...
                        _, MoveValue, VN = Star2_5(self.currentPlayer, self.Depth +1, self.MaxDepth, AX, BX).bestMove(NewState, VN)       
