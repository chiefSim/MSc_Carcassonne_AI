...
from Tile import Tile, ROTATION_DICT, SIDE_CHANGE_DICT, AvailableMove
from GameFeatures import Monastery

from Carcassonne_CityUtils import cityConnections, cityClosures
from Carcassonne_RoadUtils import roadConnections, roadClosures
from Carcassonne_FarmUtils import farmConnections
...
class CarcassonneState:
    ...
    def CloneState(self):
        ...
        Clone = CarcassonneState(self.p1, self.p2, RunInit = False)
    ...
    def IsMovePossible(self,Move):
        ...
        if TileIndex in self.TileIndexList and (X,Y) not in self.Board:
            ...
            if TestingTile is not None:
                if TestingTile.Properties[2] != PlayingTile.Properties[TileLeft]:
                    return False
            TestingTile = self.Board.get(SurroundingSpots[1])
            if TestingTile is not None:
                if TestingTile.Properties[3] != PlayingTile.Properties[TileUp]:
                    return False
            TestingTile = self.Board.get(SurroundingSpots[2])
            if TestingTile is not None:
                if TestingTile.Properties[0] != PlayingTile.Properties[TileRight]:
                    return False
            TestingTile = self.Board.get(SurroundingSpots[3])
            if TestingTile is not None:
                if TestingTile.Properties[1] != PlayingTile.Properties[TileDown]:
                    return False
            return True
        else: return False
        """
