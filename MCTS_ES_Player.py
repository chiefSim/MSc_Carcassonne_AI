...
class MCTS_ES_Player(Player):
    ...
    def ClonePlayer(self):
        Clone = MCTS_ES_Player(iterations=self.iterations, timeLimit=self.timeLimit, isTimeLimited = self.isTimeLimited, 
    ...
    def MCTS_IterationLimit(self, root_node, root_state):
        ...
        print(f'(MCTS_ES) TimeTaken: {round(endTime - startTime,3)} secs, Time:{time.strftime("%H:%M:%S", time.localtime())}')
...
def GP_Search(node, c_param, hasGPTree=False, GPTree=None):
    ...
    def evalTree(individual, nodeValues, state, childNodes):
        ...
        for i in range(SIMULATIONS):
            ...
            results.append(stateStart.checkWinner())
